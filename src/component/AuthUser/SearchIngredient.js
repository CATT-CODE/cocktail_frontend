    ...
    handleSearchOnClick = async () => {
        ...
        try {
            ...
            if ( payload.data === undefined || payload.data.length === 0 ) {
                toast.error(`No results for '${this.state.ingredientInput}', please try again`,{
                    position: "top-center",
                    autoClose: 5000,
    ...
    render() {
        return (
            <div class="container marketing">
                ...
                <div class="input-group mb-3 center-block" style={{ marginTop: 48, width: 1250, }}>
                    <input type="text" value={this.state.ingredientInput} onChange={this.handleInputOnChange} 
                        onKeyPress={event => {
                        if (event.key === "Enter") {
                            this.handleSearchOnClick()
                        }}} 
                        name="ingredientInput" class="form-control" placeholder="Input An Ingredient"/>
