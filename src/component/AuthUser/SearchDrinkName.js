...
export class SearchDrinkName extends Component {
    ...
    handleSearchOnClick = async () => {
        ...
        try {
            let payload = await axios.get(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${this.state.nameInput}`)
            if ( payload.data.drinks === null || payload.data.length === 0 ) {
                toast.error(`No results for '${this.state.nameInput}', please try again`,{
                    position: "top-center",
                    autoClose: 5000,
    ...
    render() {
        return (
            <div class="container marketing">
                <h1 style={{marginBottom: 20, marginTop:50, }} class="text-muted">Search By <span class="text-light">Drink Name</span></h1>
                <div class="input-group mb-3 center-block" style={{ marginTop: 50, width: 1250, }}>
                    <input 
                        onKeyPress={event => {
                            if (event.key === "Enter") {
                                this.handleSearchOnClick()
                            }}}
                        type="text" value={this.state.nameInput} onChange={this.handleInputOnChange}  name="nameInput" class="form-control" placeholder="Input A Drink Name">
                    </input>
                    <button type="submit" class="btn btn-outline-secondary" onClick={this.handleSearchOnClick} id="nameInput">Search</button>
                </div>
